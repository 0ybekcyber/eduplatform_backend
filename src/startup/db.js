...
require('../models/questionBankItem.model');
...
const ensureScoringColumns = async () => {
    ...
    await addColumnIfMissing(queryInterface, 'test', 'studentIds', {
        type: DataTypes.TEXT,
        allowNull: true
    });
    ...
    await addColumnIfMissing(queryInterface, 'assignment', 'studentIds', {
        type: DataTypes.TEXT,
        allowNull: true
    });
    ...
    await addColumnIfMissing(queryInterface, 'assignment_submission', 'plagiarismScore', {
        type: DataTypes.FLOAT,
        allowNull: false,
        defaultValue: 0
    });
    await addColumnIfMissing(queryInterface, 'assignment_submission', 'plagiarismReport', {
        type: DataTypes.TEXT('long'),
        allowNull: true
    });
    await addColumnIfMissing(queryInterface, 'assignment_submission', 'contentHash', {
        type: DataTypes.STRING(64),
        allowNull: true
    });
    await addColumnIfMissing(queryInterface, 'assignment_submission', 'extractedText', {
        type: DataTypes.TEXT('long'),
        allowNull: true
    });
    await addColumnIfMissing(queryInterface, 'question_bank_item', 'subject', {
