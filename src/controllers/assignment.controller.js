...
class AssignmentController extends BaseController {
    ...
    canStudentAccessAssignment = (student, assignment) => {
        ...
        const assignmentStudentIds = this.parseIds(assignment.studentIds);

        const hasFacultyLimit = assignmentFacultyIds.length > 0;
        const hasGroupLimit = assignmentGroupIds.length > 0;
        const hasStudentLimit = assignmentStudentIds.length > 0;
        ...
        const sameStudent = !hasStudentLimit || (studentId !== null && assignmentStudentIds.includes(studentId));
    ...
    create = async (req, res, next) => {
        const { name, description, maxScore, facultyIds, groupIds, studentIds, deadline, attachment } = req.body;
        ...
        const assignment = await AssignmentModel.create({
            ...
            studentIds: this.parseIds(studentIds),
            deadline,
            teacherId,
